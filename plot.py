...
TIMESTAMP_AGGR = None # 'avg' to average across timestamps for each idx/model/prompt, 'most_recent' to use the most recent timestamp for each idx/model/prompt, or None to plot all timestamps (occurs after standards aggregation)
...
if __name__ == '__main__':
    ...
    if STANDARDS_AGGR == 'avg':
        eval_df = eval_df.groupby([c for c in eval_df.columns if c not in ['standard_note_path', 'score']])['score'].mean().reset_index()
    ...
    if TIMESTAMP_AGGR == 'avg':
        eval_df = eval_df.groupby([c for c in eval_df.columns if c not in ['timestamp', 'score']])['score'].mean().reset_index()
    elif TIMESTAMP_AGGR == 'most_recent':
        timestamp_idx = eval_df.groupby([c for c in eval_df.columns if c not in ['timestamp', 'score']])['timestamp'].max().reset_index()
        eval_df = eval_df.set_index(timestamp_idx.columns.tolist()).loc[pd.MultiIndex.from_frame(timestamp_idx)].reset_index()
    
    g = sns.catplot(data=eval_df, x=X_CATEGORY, y='score', hue=COLOR_CATEGORY, col='rouge_type', row='metric_type', dodge=True, legend='full', aspect=1.5)
    g.savefig(SAVE_PATH)
